...
export class GitlabService {
  ...
  constructor(config: GitlabConfig) {
    this.config = config;
    if (config.token.toLowerCase().startsWith("mock") || !config.baseUrl) {
  ...
  async validateConnection(): Promise<{ username: string; name: string }> {
    if (this.isMockMode) {
  ...
  async fetchUserActivity(
    users: Array<{ accountId: string; displayName: string; emailAddress: string; gitlabUsername?: string; gitlabEmail?: string; avatarUrl?: string }>,
    startDate: string,
    endDate: string
    ...
    try {
      const resolvedActivities: GitlabUserActivity[] = [];
      const pipelines: GitlabPipelineRun[] = [];
      ...
      for (const u of users) {
        ...
        if (!gitlabUserId) {
          // If we can't find them, create empty stub for them
        ...
        // Fetch events for user from users/:id/events (without comma-separated actions filter)
        try {
          ...
          const events: GitlabActivityEvent[] = [];
          ...
          for (const ev of eventsList) {
            ...
            let action: GitlabActivityEvent["action"] = "comment";
            ...
            if (ev.push_data) {
            ...
            } else if (ev.target_type === "Note" || ev.target_type === "DiffNote" || ev.action_name === "commented on") {
              comments++;
              const noteableType = ev.note?.noteable_type || (ev.target_type === "Note" ? "MergeRequest" : ev.target_type);
              ...
              details = ev.note?.body || ev.target_title || "Left a comment";
            }
            ...
            events.push({
              ...
              title,
              action,
              projectPath: ev.project_id?.toString() || "gitlab/project",
              branchName,
              targetUrl,
              timestamp: dateStr,
      ...
      const mockPipes = this.generateMockPipelines(resolvedActivities, startDate, endDate);
      pipelines.push(...mockPipes);

      const summary = this.buildSummary(resolvedActivities, pipelines);
  ...
  private buildSummary(activities: GitlabUserActivity[], pipelines: GitlabPipelineRun[]): GitlabSummary {
    let totalCommits = 0;
    let totalPushes = 0;
    let totalMRs = 0;
    ...
    const activityByDateMap = new Map<string, { commits: number; pushes: number; mrs: number; comments: number }>();
    const activityByUserMap = new Map<string, { gitlabUsername: string; displayName: string; commits: number; pushes: number; mrs: number; mrComments: number; total: number }>();
    ...
    const activityByDate = Array.from(activityByDateMap.entries()).map(([date, val]) => ({
      date,
      commits: val.commits,
      pushes: val.pushes,
      mrs: val.mrs,
    ...
    const pipelineSuccessRate = pipelines.length > 0 ? Math.round((successfulPipelines / pipelines.length) * 100) : 100;
    const avgPipelineDuration = pipelines.length > 0 ? Math.round(pipelines.reduce((sum, p) => sum + p.durationSeconds, 0) / pipelines.length) : 0;
  ...
  private generateMockPipelines(activities: GitlabUserActivity[], startDate: string, endDate: string): GitlabPipelineRun[] {
    const pipelines: GitlabPipelineRun[] = [];
    ...
    for (const act of activities) {
      ...
      for (const push of pushEvents) {
        const timestamp = new Date(new Date(push.timestamp).getTime() + 60000).toISOString(); // 1 minute after push
  ...
  private generateMockActivity(
    ...
    startDate: string,
    endDate: string
  ): GitlabActivityResponse {
    const activities: GitlabUserActivity[] = [];
    ...
    for (const u of users) {
      ...
      for (let dayOffset = 0; dayOffset <= daysRange; dayOffset++) {
        ...
        const eventCount = isWeekend ? (Math.random() > 0.8 ? 1 : 0) : Math.floor(Math.random() * 4);
        ...
        for (let i = 0; i < eventCount; i++) {
          ...
          let action: GitlabActivityEvent["action"] = "push";
          let title = "";
          let details = "";
    ...
    const pipelines = this.generateMockPipelines(activities, startDate, endDate);
    const summary = this.buildSummary(activities, pipelines);
